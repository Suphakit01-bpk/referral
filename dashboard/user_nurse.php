...
<head>
    ...
    <style>
        ...
        .nav-buttons {
            margin-left: auto;
            /* This pushes the div to the right */
            display: flex;
            gap: 10px;
        ...
        .nav-button {
            /* Your existing nav-button styles */
        }

        .logout-button {
            /* Remove margin-left: auto from here if it exists */
            background-color: #dc3545;
        }
    </style>
</head>
        ...
        <div class="table-container">
            ...
            <div class="header-left">
                    ...
                    <div class="table-toggle">
                        ...
                        <button id="incoming-button">ผู้ป่วยที่ส่งมา</button>
                    </div>
                </div>
                ...
                <div id="popup-form" class="popup-form hidden">
                    <div class="popup-content">
                        ...
                        <form id="transfer-form">
                            ...
                            <select id="approved-hospital" name="approved-hospital" required>
                                <option value="โรงพยาบาลบางปะกอก 9 อินเตอร์เนชั่นแนล" <?php echo ($hospital === 'โรงพยาบาลบางปะกอก 9 อินเตอร์เนชั่นแนล') ? 'selected' : ''; ?>>โรงพยาบาลบางปะกอก 9 อินเตอร์เนชั่นแนล</option>
                                <option value="โรงพยาบาลบางปะกอก 1" <?php echo ($hospital === 'โรงพยาบาลบางปะกอก 1') ? 'selected' : ''; ?>>โรงพยาบาลบางปะกอก 1</option>
                                <option value="โรงพยาบาลบางปะกอก 3" <?php echo ($hospital === 'โรงพยาบาลบางปะกอก 3') ? 'selected' : ''; ?>>โรงพยาบาลบางปะกอก 3</option>
                                <option value="โรงพยาบาลบางปะกอก 8" <?php echo ($hospital === 'โรงพยาบาลบางปะกอก 8') ? 'selected' : ''; ?>>โรงพยาบาลบางปะกอก 8</option>
                                <option value="โรงพยาบาลบางปะกอก 2 รังสิต" <?php echo ($hospital === 'โรงพยาบาลบางปะกอก 2 รังสิต') ? 'selected' : ''; ?>>โรงพยาบาลบางปะกอก 2 รังสิต</option>
                                <option value="โรงพยาบาลบางปะกอกสมุทรปราการ" <?php echo ($hospital === 'โรงพยาบาลบางปะกอกสมุทรปราการ') ? 'selected' : ''; ?>>โรงพยาบาลบางปะกอกสมุทรปราการ</option>
                                <option value="โรงพยาบาลปิยะเวท" <?php echo ($hospital === 'โรงพยาบาลปิยะเวท') ? 'selected' : ''; ?>>โรงพยาบาลปิยะเวท</option>
                ...
                fetch(`../action_dashboard/get_transfer.php?national_id=${nationalId}&id=${id}`)
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            document.getElementById('approved-hospital').value = record.approved_hospital || '<?php echo htmlspecialchars($hospital); ?>';
                            ...
                            Swal.fire({
                                ...
                                text: 'ไม่สามารถดึงข้อมูลได้: ' + data.error,
                                icon: 'error',
                    ...
                    if (result.isConfirmed) {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                Swal.fire({
                                    title: 'สำเร็จ',
                                    text: 'ยกเลิกการส่งตัวเรียบร้อยแล้ว',
                                    icon: 'success',
                                    timer: 1500,
                                    showConfirmButton: false
                                });
                        ...
                        .catch(error => {
                            Swal.fire({
                                title: 'เกิดข้อผิดพลาด',
                                text: 'ไม่สามารถยกเลิกการส่งตัวได้: ' + error.message,
                                icon: 'error',
                                confirmButtonText: 'ตกลง'
                            });
                        });
...
<script>
        document.addEventListener('DOMContentLoaded', function() {
            const outgoingButton = document.getElementById('outgoing-button');
            const incomingButton = document.getElementById('incoming-button');
            ...
            function switchTable(showIncoming) {
                ...
                if (showIncoming) {
                    ...
                    incomingButton.classList.add('active');
                    outgoingButton.classList.remove('active');
                    addFormButton.style.display = 'none';
                    fetchIncomingData();
                } else {
                    outgoingTable.style.display = 'block';
                    incomingTable.style.display = 'none';
                    outgoingButton.classList.add('active');
                    incomingButton.classList.remove('active');
                    addFormButton.style.display = 'block';
                    fetchData(); // ฟังก์ชันเดิมสำหรับดึงข้อมูลผู้ป่วยที่ส่งออก
                }
            }
            ...
            // เพิ่มฟังก์ชันสำหรับการอนุมัติและปฏิเสธ
            function approveTransfer(nationalId) {
                if (confirm('คุณต้องการอนุมัติการส่งตัวผู้ป่วยนี้ใช่หรือไม่?')) {
                    updateTransferStatus(nationalId, 'อนุมัติแล้ว');
                }
            }

            function rejectTransfer(nationalId) {
                if (confirm('คุณต้องการปฏิเสธการส่งตัวผู้ป่วยนี้ใช่หรือไม่?')) {
                    updateTransferStatus(nationalId, 'ปฏิเสธ');
                }
            }

            function updateTransferStatus(nationalId, status) {
                fetch('../action_dashboard/update_status.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                ...
                .then(data => {
                    if (data.success) {
                        fetchIncomingData(); // รีเฟรชตาราง
                        const toast = document.getElementById('toast');
                        toast.querySelector('.toast-title').textContent = 'ดำเนินการสำเร็จ';
                        toast.querySelector('.toast-description').textContent = 
                            `การส่งตัวผู้ป่วยถูก${status}เรียบร้อยแล้ว`;
            ...
            // Event listeners
            outgoingButton.addEventListener('click', () => switchTable(false));
